...
const seedEvents = [
  {
    id: "crimea-queue",
    type: "station",
    title: T("Crimea: queues, limits, vouchers", "Крым: очереди, лимиты, талоны"),
    region: T("Crimea", "Крым"),
    place: T("Simferopol / peninsula roads", "Симферополь / трассы полуострова"),
    lat: 44.95,
    lng: 34.1,
    severity: "critical",
    status: T("Queues, dry pumps, vouchers", "Очереди, сухие колонки, талоны"),
  ...
  },
  {
    ...
    type: "station",
    title: T("Moscow Oblast: selected-station queue reports", "Московская область: сообщения об очередях на отдельных АЗС"),
    region: T("Moscow Oblast", "Московская область"),
    place: T("Western districts", "Западные районы области"),
    lat: 55.55,
    lng: 36.7,
    severity: "watch",
    status: T("Needs video-level verification", "Требует проверки по видео"),
    fuel: T("AI-95 / diesel", "АИ-95 / дизель"),
    lossWeight: 3,
...
const regionStress = [
  { region: T("Crimea", "Крым"), lat: 45.3, lng: 34.4, severity: "critical", score: 92, affectedStations: 180, note: T("Queues, vouchers and sales restrictions.", "Очереди, талоны, ограничения продаж.") },
  ...
  { region: T("Moscow Oblast", "Московская область"), lat: 55.7, lng: 37.3, severity: "watch", score: 38, affectedStations: 60, note: T("Queues and limits need point-level verification.", "Очереди и лимиты требуют точечной верификации.") },
  ...
  { region: T("Nationwide shortage reports", "Общероссийские сообщения о дефиците"), lat: 58.5, lng: 76.0, severity: "watch", score: 42, affectedStations: 900, note: T("Baseline aggregation for reports across dozens of regions. Uploaded videos confirm local points.", "Базовая агрегация сообщений по десяткам регионов. Загруженные видео подтверждают локальные точки.") },
];
